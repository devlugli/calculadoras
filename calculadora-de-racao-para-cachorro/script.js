...
function obterFatorDog(fase, atividade) {
  const fatores = {
    filhote: { baixa: 2.0, moderada: 2.4, alta: 2.8 },
    adulto: { baixa: 1.4, moderada: 1.6, alta: 2.0 },
    idoso: { baixa: 1.2, moderada: 1.4, alta: 1.6 }
  };
  return fatores[fase][atividade];
}

function obterRefeicoesDog(fase) {
...
function calcular() {
  ...
  const fator = obterFatorDog(fase, atividade);
  const kcalDia = rer * fator;
  const gramasDia = kcalDia / (kcalPorKg / 1000);
  const refeicoes = obterRefeicoesDog(fase);
  const porRefeicao = gramasDia / refeicoes;

  resumoTopoValor.innerText = formatarNumero(gramasDia) + " g/dia";
  resumoTopoTexto.innerText = `Para um cachorro de ${formatarNumero(peso, 1)} kg, a estimativa diária é de ${formatarNumero(gramasDia)} g de ração, considerando ${atividade} atividade e fase ${fase}.`;
